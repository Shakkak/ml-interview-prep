...
Almost universally a **Vision Transformer (ViT)** pretrained with contrastive learning (usually CLIP or similar). Common choices:
...
1. **MLP connector instead of linear**: a two-layer MLP with GELU activation. Adds mild non-linearity; empirically helps ~4 points on benchmarks.
...
The visual encoder (NFNet) outputs a variable-length feature grid. Before feeding to the LLM, Flamingo uses a **Perceiver Resampler**: a cross-attention module that maps the variable-length visual features to a fixed set of 64 learned "query" tokens.
...
[[clip]], [[blip]], [[open-vocabulary-detection]], [[remote-sensing-foundation-models]], [[vlm-explainability]], [[biodiversity-ml]], [[lora-quantization]], [[vision-transformer]]
